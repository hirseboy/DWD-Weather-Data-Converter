...
DataMapWidget::DataMapWidget(QWidget *parent) :
	QWidget(parent),
	m_ui(new Ui::DataMapWidget)
{
	m_ui->setupUi(this);
	...
	connect(m_scene, &DM::Scene::updatedLocation, this, &DM::DataMapWidget::onUpdateLocation);
	// draw al data points
...
void DataMapWidget::setCoordinates(double latitude, double longitude) {
	...
	QPointF pos = convertCoordinatesToPos(m_scene->m_mapSvgItem->boundingRect(), m_latitude, m_longitude);
	m_scene->m_locationItem->setPos(pos);
}

void DataMapWidget::setDistance(double distance) {
...
void DataMapWidget::onUpdateLocation() {
...
void DataMapWidget::on_checkBoxAirTemp_toggled(bool checked) {
	m_scene->setItemGroupVisiblity(Data::DT_TemperatureAndHumidity, checked);
}


void DataMapWidget::on_checkBoxRadiation_toggled(bool checked) {
	m_scene->setItemGroupVisiblity(Data::DT_Solar, checked);
}


void DataMapWidget::on_checkBoxPressure_toggled(bool checked) {
	m_scene->setItemGroupVisiblity(Data::DT_Pressure, checked);
}


void DataMapWidget::on_checkBoxWind_toggled(bool checked) {
	m_scene->setItemGroupVisiblity(Data::DT_Wind, checked);
}


void DataMapWidget::on_checkBoxPrecipitation_toggled(bool checked) {
	m_scene->setItemGroupVisiblity(Data::DT_Precipitation, checked);
}

void DataMapWidget::showEvent(QShowEvent * event) {
	// fit scene in view
...
void DataMapWidget::on_lineEditLongitude_editingFinishedSuccessfully() {
...
void DataMapWidget::on_lineEditLatitude_editingFinishedSuccessfully() {
	double lati = m_ui->lineEditLatitude->value();
	m_longitude = lati;

	QPointF pos = convertCoordinatesToPos(m_scene->m_mapSvgItem->boundingRect(), lati, m_longitude);
	m_scene->m_locationItem->setPos(pos);
...
void DataMapWidget::on_horizontalSliderDistance_valueChanged(int distance) {
